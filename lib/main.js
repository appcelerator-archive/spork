...
var ALLOY_DIR = '/app/assets/';
var TI_DIR = '/Resources/';

// Get's the root directory for the assets based on whether this is an alloy app or not.
function getRootDir(projectDir) {
    var isAlloyProject = shell.test('-d', projectDir + '/app');
    return projectDir + (isAlloyProject ? ALLOY_DIR : TI_DIR); 
}

// Normalizes the project directory passed in.
...
program.command('icons <project-dir>')
    ...
    .action(function IconsCommand(projectDir, options) { 
        ...
        var execOptions = { cwd: getRootDir(projectDir), debug: program.debug };
...
program.command('splashes <project-dir>')
    ...
    .action(function SplashesCommand(projectDir, options) {  
        ...
        if (program.android) {
            var execOptions = { cwd: getRootDir(projectDir) + 'android/images/', debug: program.debug };
        ...
        if (program.ios) {
            var execOptions = { cwd: getRootDir(projectDir) + 'iPhone/', debug: program.debug };
            ...
            // iPhone Landscape    
            cmds = [];
            cmds.push({ cmd: 'convert ' + inDir + 'Landscape.png -crop 2048x1496+0+20 Default-Landscape@2x.png', options: execOptions });
    ...
    .action(function SplashesCommand(projectDir, bkgndFolder, options) {  
        ...
        fs.readdir(inDir, function(error, curFiles) {
            ...
            for (var i = 0; i < curFiles.length; i++) {
                ...
                if (program.android) {     
                    execOptions = { cwd: getRootDir(projectDir) + 'android/images/', debug: program.debug };
                ...
                if (program.ios) {
                    execOptions = { cwd: getRootDir(projectDir) + 'iphone/images/' + bkgndFolder + '/', debug: program.debug };
                    wrench.mkdirSyncRecursive(execOptions.cwd);
                ...
                if (program.mobileweb) {
                    if (isPortrait || isLandscape) {
                        execOptions = { cwd: getRootDir(projectDir) + 'mobileweb/images/' + bkgndFolder + '/', debug: program.debug };
                        wrench.mkdirSyncRecursive(execOptions.cwd);
...
program.command('assets <project-dir> <assets-folder> <width>')
    ...
    .action(function AssetsCommand(projectDir, assetFolder, width, options) {  
        ...
        if (program.android) {
            var outDir = getRootDir(projectDir) + 'android/images/';                        
        ...
        if (program.ios) {
            ...
            wrench.mkdirSyncRecursive(getRootDir(projectDir) + 'iphone/images/' + assetFolder);
        }
        
        if (program.mobileweb) {
            console.log('TiSpork assets: Mobile Web generating images with no scaling');
            wrench.mkdirSyncRecursive(getRootDir(projectDir) + 'mobileweb/images/' + assetFolder);
        }     
        ...
        fs.readdir(assetsDir, function(error, curFiles) {
            ...
            for (var i = 0; i < curFiles.length; i++) {
                ...
                if (program.ios) {
                    execOptions = { cwd: getRootDir(projectDir) + 'iphone/images/' + assetFolder + '/', debug: program.debug };
                    if (options.retina) {
                ...
                if (program.mobileweb) {
                    execOptions = { cwd: getRootDir(projectDir) + 'mobileweb/images/' + assetFolder + '/', debug: program.debug };
