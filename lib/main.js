...
program.command('icons <project-dir>')
    ...
    .action(function IconsCommand(projectDir, options) { 
        ...
        var alloyProject = shell.test('-d', projectDir + '/app');
        console.log('TiSpork icons: generating icons for ' + (alloyProject ? 'Titanium Alloy project' : 'standard Titanium project'));
        var execOptions = { cwd: projectDir + (alloyProject ? '/app/assets' : '/Resources'), debug: program.debug };
...
program.command('splashes <project-dir>')
    ...
    .action(function SplashesCommand(projectDir, options) {  
        ...
        var alloyProject = shell.test('-d', projectDir + 'app');
        console.log('TiSpork splashes: generating splashes for ' + (alloyProject ? 'Titanium Alloy project' : 'standard Titanium project'));
        ...
        if (program.android) {
            var execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/android/images/' : 'Resources/android/images/'), debug: program.debug };
        ...
        if (program.ios) {
            var execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/iPhone/' : 'Resources/iPhone/'), debug: program.debug };
            wrench.mkdirSyncRecursive(execOptions.cwd);
            console.log('TiSpork splashes: generating for iOS--' + execOptions.cwd);
    
            // iPhone Portrait
            cmds = [];
            cmds.push({ cmd: 'convert ' + inDir + 'Portrait.png -resize 1536x2008! Default-Portrait@2x.png', options: execOptions }); // iPad 3
            cmds.push({ cmd: 'convert Default-Portrait@2x.png -scale 50% Default-Portrait.png', options: execOptions }); // iPad 1,2
            temp = 'default-temp-ip.png';
            cmds.push({ cmd: 'convert Default-Portrait@2x.png -crop 1339x2008+99 ' + temp, options: execOptions }); 
            cmds.push({ cmd: 'convert ' + temp + ' -resize 640x960! Default@2x.png', options: execOptions });   // iPhone 4
            cmds.push({ cmd: 'convert ' + temp + ' -resize 320x480! Default.png', options: execOptions });      // iPhone 3
            cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
            temp = 'default-temp-ip5.png';
            cmds.push({ cmd: 'convert Default-Portrait@2x.png -crop 1131x2008+202 ' + temp, options: execOptions }); 
            cmds.push({ cmd: 'convert ' + temp + ' -resize 640x1136! Default-568h@2x.png', options: execOptions });   // iPhone 5
            cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
            exec.sync(cmds, execResults);
     
            // iPhone Landscape    
            cmds = [];
            cmds.push({ cmd: 'convert ' + inDir + 'Landscape.png -crop 2048x1496+20 Default-Landscape@2x.png', options: execOptions });
...
program.command('backgrounds <project-dir> <bkgnd-folder>')
    .description('Traverse the <project-dir>/images/<bkgnd-folder> directory and generate cropped and scaled background images appropriate to the device in the right device specific folders. Images must be suffixed with -Portrait or -Landscape.')
    .action(function SplashesCommand(projectDir, bkgndFolder, options) {  
        ...
        var alloyProject = shell.test('-d', projectDir + 'app');
        console.log('TiSpork backgrounds: generating backgrounds for ' + (alloyProject ? 'Titanium Alloy project' : 'standard Titanium project'));
        ...
        fs.readdir(inDir, function(error, curFiles) {
            ...
            for (var i = 0; i < curFiles.length; i++) {
                ...
                console.log("--" + curFiles[i] + (isPortrait || isLandscape ? ' processing' : ' skipped'));                
                
                if (program.android) {     
                    execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/android/images/' : 'Resources/android/images/'), debug: program.debug };
                    ...
                    wrench.mkdirSyncRecursive(execOptions.cwd + 'res-xhdpi/' + bkgndFolder);
                    ...
                    if (isPortrait) {
                        ...
                        cmds.push({ cmd: 'convert ' + inDir + curFiles[i] + ' -crop 1205x2008+166 ' + temp, options: execOptions });
                        ...
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 960x1600! res-xhdpi/' + bkgndFolder + '/' + outFile + '-Portrait.png', options: execOptions });
                        cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
                 
                        temp = 'temp-ap2.png';      // 3/4 Aspect ratio
                        cmds.push({ cmd: 'convert ' + inDir + curFiles[i] + ' -crop 1506x2008+15 ' + temp, options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 240x320! low/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 480x640! medium/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 480x640! high/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 960x1280! res-xhdpi/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
 
                        temp = 'temp-ap3.png'       // 2/3 Aspect ratio
                        cmds.push({ cmd: 'convert ' + inDir + curFiles[i] + ' -crop 1339x2008+99 ' + temp, options: execOptions });
                    ...
                    if (isLandscape) {           
                        ...
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 640x480! medium/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 640x480! high/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 1280x960! res-xhdpi/' + bkgndFolder + '/' + outFile + '-Portrait-Short.png', options: execOptions });
                        cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
                ...
                if (program.ios) {
                    execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/iphone/images/' : 'Resources/iphone/images/') + bkgndFolder + '/', debug: program.debug };
                    wrench.mkdirSyncRecursive(execOptions.cwd);
                    ...
                    if (isPortrait) {
                        ...
                        cmds = [];                  // 0.76 Aspect ratio iPad
                        cmds.push({ cmd: 'convert ' +  inDir + curFiles[i] + ' -crop 1536x2008 ' + outFile + '-Portrait-Tablet@2x.png', options: execOptions });
                        cmds.push({ cmd: 'convert ' + outFile + '-Portrait-Tablet@2x.png -scale 50% ' + outFile + '-Portrait-Tablet.png', options: execOptions });
 
                        temp = 'temp-ip1.png';      // 2/3 Aspect ratio iPhone
                        cmds.push({ cmd: 'convert ' +  inDir + curFiles[i] + ' -crop 1339x2048+99 ' + temp, options: execOptions });
                        ...
                        temp = 'temp-ip5.png';      // 0.56 Aspect ratio iPhone 5                     
                        cmds.push({ cmd: 'convert ' +  inDir + curFiles[i] + ' -crop 1131x2048+202 ' + temp, options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 640x1136! ' + outFile + '-Portrait-Phone-568h@2x.png', options: execOptions });
                        cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
                    ...
                    if (isLandscape) {
                        ...
                        temp = 'temp-il1.png';      // 3/2 Aspect ratio iPhone
                        cmds.push({ cmd: 'convert ' + inDir + curFiles[i] + ' -crop 2048x1365+0+85 ' + temp, options: execOptions });
                        ...
                        temp = 'temp-il5.png';      // 3/2 Aspect ratio iPhone
                        cmds.push({ cmd: 'convert ' + inDir + curFiles[i] + ' -crop 2048x1154+0+191 ' + temp, options: execOptions });
                        cmds.push({ cmd: 'convert ' + temp + ' -resize 1136x640 ' + outFile + '-Landscape-Phone-568h@2x.png', options: execOptions });
                        cmds.push({ cmd: 'rm ' + temp, options: execOptions }); 
                ...
                if (program.mobileweb) {
                    if (isPortrait || isLandscape) {
                        execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/mobileweb/images/' : 'Resources/mobileweb/images/' + bkgndFolder + '/'), debug: program.debug };
                        wrench.mkdirSyncRecursive(execOptions.cwd);
...
program.command('assets <project-dir> <assets-folder> <width>')
    ...
    .action(function AssetsCommand(projectDir, assetFolder, width, options) {  
        ...
        var alloyProject = shell.test('-d', projectDir + 'app');
        console.log('TiSpork assets: generating assets for ' + (alloyProject ? 'Titanium Alloy project' : 'standard Titanium project'));
        ...
        if (program.android) {
            var outDir = projectDir + (alloyProject ? 'app/assets/android/images/' : 'Resources/android/images/');                        
        ...
        if (program.ios) {
            ...
            wrench.mkdirSyncRecursive(projectDir + (alloyProject ? 'app/assets/iphone/images/' : 'Resources/iphone/images/') + assetFolder);
        }
        
        if (program.mobileweb) {
            console.log('TiSpork assets: Mobile Web generating images with no scaling');
            wrench.mkdirSyncRecursive(projectDir + (alloyProject ? 'app/assets/mobileweb/images/' : 'Resources/mobileweb/images/') + assetFolder);
        }     
        ...
        fs.readdir(assetsDir, function(error, curFiles) {
            ...
            for (var i = 0; i < curFiles.length; i++) {
                ...
                if (program.ios) {
                    execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/iphone/images/' : 'Resources/iphone/images/') + assetFolder + '/', debug: program.debug };
                    if (options.retina) {
                ...
                if (program.mobileweb) {
                    execOptions = { cwd: projectDir + (alloyProject ? 'app/assets/mobileweb/images/' : 'Resources/mobileweb/images/') + assetFolder + '/', debug: program.debug };
...
program.command('*')
    .action(function AssetsGeneral(options) { 
        console.log('TiSpork: unknown command:', options); 
    });
